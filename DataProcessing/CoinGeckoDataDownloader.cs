...
namespace QuantConnect.DataProcessing
{
    ...
    public class CoinGeckoUniverseDataDownloader : IDisposable
    {
        ...
        public CoinGeckoUniverseDataDownloader(string destinationFolder, string processedFolder, DateTime processDate)
        {
            ...
            _blacklistFile = Path.Combine(_destinationFolder, "blacklist.csv");
            var sourceFileName = Path.Combine(processedFolder, "blacklist.csv");
            if (File.Exists(sourceFileName)) File.Copy(sourceFileName, _blacklistFile);
        ...
        public bool Run()
        {
            ...
            foreach (var (ticker, coinGeckoList) in coinGeckoListByTicker)
            {
                ...
                var marketChartsById = coinGeckoList.ToDictionary(x => x.Id, x => GetCoinGeckoMarketChartsForId(x.Id, exists));
